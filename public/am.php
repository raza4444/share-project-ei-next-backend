...
script("$gh.onclick = function () { return confirm('".($Le?js_escape($Le):'Sind Sie sicher?')."'); };","");}function
...
upload_error($n){$Ie=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?'Hochladen von Datei fehlgeschlagen.'.($Ie?" ".sprintf('Maximal erlaubte DateigrÃ¶ÃŸe ist %sB.',$Ie):""):'Datei existiert nicht.');}function
...
h($g->error);}function
check_sqlite_name($B){global$g;$Kc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Kc)\$~",$B)){$g->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$Kc));return
false;}return
true;}function
...
Min_DB;$Hb=$b->credentials();if($g->connect($Hb[0],$Hb[1],$Hb[2])){if(min_version(9,0,$g)){$g->query("SET application_name = 'Adminer'");if(min_version(9.2,0,$g)){$Hh['Zeichenketten'][]="json";$U["json"]=4294967295;if(min_version(9.4,0,$g)){$Hh['Zeichenketten'][]="jsonb";$U["jsonb"]=4294967295;}}}return$g;}return$g->error;}function
get_databases(){return
...
set_schema($ah,$h=null){global$g,$U,$Hh;if(!$h)$h=$g;$H=$h->query("SET search_path TO ".idf_escape($ah));foreach(types()as$T){if(!isset($U[$T])){$U[$T]=0;$Hh['Benutzerdefinierte Typen'][]=$T;}}return$H;}function
...
unconvert_field($o,$H){return$H;}function
support($Pc){return
preg_match('~^(columns|database|drop_col|indexes|descidx|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Pc);}$x="oracle";$U=array();$Hh=array();foreach(array('Zahlen'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum und Zeit'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Zeichenketten'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'BinÃ¤r'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$U+=$X;$Hh[$y]=array_keys($X);}$Ii=array();$vf=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$kd=array("length","lower","round","upper");$qd=array("avg","count","count distinct","max","min","sum");$mc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$ec["mssql"]="MS SQL (beta)";if(isset($_GET["mssql"])){$hg=array("SQLSRV","MSSQL","PDO_DBLIB");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
last_id(){global$g;return$g->last_id;}$x="elastic";$vf=array("=","query");$kd=array();$qd=array();$mc=array(array("json"));$U=array();$Hh=array();foreach(array('Zahlen'=>array("long"=>3,"integer"=>5,"short"=>8,"byte"=>10,"double"=>20,"float"=>66,"half_float"=>12,"scaled_float"=>21),'Datum und Zeit'=>array("date"=>10),'Zeichenketten'=>array("string"=>65535,"text"=>65535),'BinÃ¤r'=>array("binary"=>255),)as$y=>$X){$U+=$X;$Hh[$y]=array_keys($X);}}$ec["clickhouse"]="ClickHouse (alpha)";if(isset($_GET["clickhouse"])){define("DRIVER","clickhouse");class
...
0;}function
support($Pc){return
preg_match("~^(columns|sql|status|table|drop_col)$~",$Pc);}$x="clickhouse";$U=array();$Hh=array();foreach(array('Zahlen'=>array("Int8"=>3,"Int16"=>5,"Int32"=>10,"Int64"=>19,"UInt8"=>3,"UInt16"=>5,"UInt32"=>10,"UInt64"=>20,"Float32"=>7,"Float64"=>16,'Decimal'=>38,'Decimal32'=>9,'Decimal64'=>18,'Decimal128'=>38),'Datum und Zeit'=>array("Date"=>13,"DateTime"=>20),'Zeichenketten'=>array("String"=>0),'BinÃ¤r'=>array("FixedString"=>0),)as$y=>$X){$U+=$X;$Hh[$y]=array_keys($X);}$Ii=array();$vf=array("=","<",">","<=",">=","!=","~","!~","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","SQL");$kd=array();$qd=array("avg","count","count distinct","max","min","sum");$mc=array();}$ec=array("server"=>"MySQL")+$ec;if(!defined("DRIVER")){$hg=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
...
selectLinks($Oh,$N=""){global$x,$m;echo'<p class="links">';$we=array("select"=>'Daten auswÃ¤hlen');if(support("table")||support("indexes"))$we["table"]='Struktur anzeigen';if(support("table")){if(is_view($Oh))$we["view"]='View Ã¤ndern';else$we["create"]='Tabelle Ã¤ndern';}if($N!==null)$we["edit"]='Neuer Datensatz';$B=$Oh["Name"];foreach($we
...
selectColumnsPrint($K,$f){global$kd,$qd;print_fieldset("select",'Daten zeigen von',$K);$s=0;$K[""]=array();foreach($K
as$y=>$X){$X=$_GET["columns"][$y];$e=select_input(" name='columns[$s][col]'",$f,$X["col"],($y!==""?"selectFieldChange":"selectAddRow"));echo"<div>".($kd||$qd?"<select name='columns[$s][fun]'>".optionlist(array(-1=>"")+array_filter(array('Funktionen'=>$kd,'Aggregationen'=>$qd)),$X["fun"])."</select>".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).script("qsl('select').onchange = function () { helpClose();".($y!==""?"":" qsl('select, input', this.parentNode).onchange();")." };","")."($e)":$e)."</div>\n";$s++;}echo"</div></fieldset>\n";}function
...
selectOrderPrint($_f,$f,$w){print_fieldset("sort",'Ordnen',$_f);$s=0;foreach((array)$_GET["order"]as$y=>$X){if($X!=""){echo"<div>".select_input(" name='order[$s]'",$f,$X,"selectFieldChange"),checkbox("desc[$s]",1,isset($_GET["desc"][$y]),'absteigend')."</div>\n";$s++;}}echo"<div>".select_input(" name='order[$s]'",$f,"","selectAddRow"),checkbox("desc[$s]",1,false,'absteigend')."</div>\n","</div></fieldset>\n";}function
...
selectLengthPrint($di){if($di!==null){echo"<fieldset><legend>".'TextlÃ¤nge'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($di)."'>","</div></fieldset>\n";}}function
...
messageQuery($F,$ei,$Nc=false){global$x,$m;restart_session();$yd=&get_session("queries");if(!$yd[$_GET["db"]])$yd[$_GET["db"]]=array();if(strlen($F)>1e6)$F=preg_replace('~[\x80-\xFF]+$~','',substr($F,0,1e6))."\nâ€¦";$yd[$_GET["db"]][]=array($F,time(),$ei);$Ah="sql-".count($yd[$_GET["db"]]);$H="<a href='#$Ah' class='toggle'>".'SQL-Kommando'."</a>\n";if(!$Nc&&($dj=$m->warnings())){$t="warnings-".count($yd[$_GET["db"]]);$H="<a href='#$t' class='toggle'>".'Warnings'."</a>, $H<div id='$t' class='hidden'>\n$dj</div>\n";}return" <span class='time'>".@date("H:i:s")."</span>"." $H<div id='$Ah' class='hidden'><pre><code class='jush-$x'>".shorten_utf8($F,1000)."</code></pre>".($ei?" <span class='time'>($ei)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($yd[$_GET["db"]])-1)).'">'.'Bearbeiten'.'</a>':'').'</div>';}function
editFunctions($o){global$mc;$H=($o["null"]?"NULL/":"");foreach($mc
as$y=>$kd){if(!$y||(!isset($_GET["call"])&&(isset($_GET["select"])||where($_GET)))){foreach($kd
as$ag=>$X){if(!$ag||preg_match("~$ag~",$o["type"]))$H.="/$X";}if($y&&!preg_match('~set|blob|bytea|raw|file~',$o["type"]))$H.="/SQL";}}if($o["auto_increment"]&&!isset($_GET["select"])&&!where($_GET))$H='Auto-Inkrement';return
explode("/",$H);}function
...
check_invalid_login(){global$b;$Wd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Vd=$Wd[$b->bruteForceKey()];$cf=($Vd[1]>29?$Vd[0]-time():0);if($cf>0)auth_error(lang(array('Zu viele erfolglose Login-Versuche. Bitte probieren Sie es in %d Minute noch einmal.','Zu viele erfolglose Login-Versuche. Bitte probieren Sie es in %d Minuten noch einmal.'),ceil($cf/60)));}$Ka=$_POST["auth"];if($Ka){session_regenerate_id();$Xi=$Ka["driver"];$M=$Ka["server"];$V=$Ka["username"];$E=(string)$Ka["password"];$l=$Ka["db"];set_password($Xi,$M,$V,$E);$_SESSION["db"][$Xi][$M][$V][$l]=true;if($Ka["permanent"]){$y=base64_encode($Xi)."-".base64_encode($M)."-".base64_encode($V)."-".base64_encode($l);$ng=$b->permanentLogin(true);$bg[$y]="$y:".base64_encode($ng?encrypt_string($E,$ng):"");cookie("adminer_permanent",implode(" ",$bg));}if(count($_POST)==1||DRIVER!=$Xi||SERVER!=$M||$_GET["username"]!==$V||DB!=$l)redirect(auth_url($Xi,$M,$V,$l));}elseif($_POST["logout"]){if($vd&&!verify_token()){page_header('Abmelden','CSRF Token ungÃ¼ltig. Bitte die Formulardaten erneut abschicken.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Abmeldung erfolgreich.'.' '.'Danke, dass Sie Adminer genutzt haben. <a href="https://www.adminer.org/de/donation/">Spenden willkommen!</a>');}}elseif($bg&&!$_SESSION["pwds"]){session_regenerate_id();$ng=$b->permanentLogin();foreach($bg
...
edit_type($y,$o,$pb,$ed=array(),$Lc=array()){global$Hh,$U,$Ii,$qf;$T=$o["type"];echo'<td><select name="',h($y),'[type]" class="type" aria-labelledby="label-type">';if($T&&!isset($U[$T])&&!isset($ed[$T])&&!in_array($T,$Lc))$Lc[]=$T;if($ed)$Hh['FremdschlÃ¼ssel']=$ed;echo
optionlist(array_merge($Lc,$Hh),$T),'</select>',on_help("getTarget(event).value",1),script("mixin(qsl('select'), {onfocus: function () { lastType = selectValue(this); }, onchange: editingTypeChange});",""),'<td><input name="',h($y),'[length]" value="',h($o["length"]),'" size="3"',(!$o["length"]&&preg_match('~var(char|binary)$~',$T)?" class='required'":"");echo' aria-labelledby="label-length">',script("mixin(qsl('input'), {onfocus: editingLengthFocus, oninput: editingLengthChange});",""),'<td class="options">',"<select name='".h($y)."[collation]'".(preg_match('~(char|text|enum|set)$~',$T)?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($pb,$o["collation"]).'</select>',($Ii?"<select name='".h($y)."[unsigned]'".(!$T||preg_match(number_type(),$T)?"":" class='hidden'").'><option>'.optionlist($Ii,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($y)."[on_update]'".(preg_match('~timestamp|datetime~',$T)?"":" class='hidden'").'>'.optionlist(array(""=>"(".'ON UPDATE'.")","CURRENT_TIMESTAMP"),(preg_match('~^CURRENT_TIMESTAMP~i',$o["on_update"])?"CURRENT_TIMESTAMP":$o["on_update"])).'</select>':''),($ed?"<select name='".h($y)."[on_delete]'".(preg_match("~`~",$T)?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$qf),$o["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td id='label-comment'>".'Kommentar':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.7.5")."' alt='+' title='".'HinzufÃ¼gen'."'>".script("row_count = ".count($p).";"),'</thead>
<tbody>
...
',($T=="PROCEDURE"?"<td>".html_select("fields[$s][inout]",explode("|",$Rd),$o["inout"]):""),'<th>';if($bc){echo'<input name="fields[',$s,'][field]" value="',h($o["field"]),'" data-maxlength="64" autocapitalize="off" aria-labelledby="label-name">',script("qsl('input').oninput = function () { editingNameChange.call(this);".($o["field"]!=""||count($p)>1?"":" editingAddRow.call(this);")." };","");}echo'<input type="hidden" name="fields[',$s,'][orig]" value="',h($Df),'">';edit_type("fields[$s]",$o,$pb,$ed);if($T=="TABLE"){echo'<td>',checkbox("fields[$s][null]",1,$o["null"],"","","block","label-null"),'<td><label class="block"><input type="radio" name="auto_increment_col" value="',$s,'"';if($o["auto_increment"]){echo' checked';}echo' aria-labelledby="label-ai"></label><td>',checkbox("fields[$s][has_default]",1,$o["has_default"],"","","","label-default"),'<input name="fields[',$s,'][default]" value="',h($o["default"]),'" aria-labelledby="label-default">',(support("comment")?"<td><input name='fields[$s][comment]' value='".h($o["comment"])."' data-maxlength='".(min_version(5.5)?1024:255)."' aria-labelledby='label-comment'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.7.5")."' alt='+' title='".'HinzufÃ¼gen'."'> "."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=up.gif&version=4.7.5")."' alt='â†‘' title='".'Nach oben'."'> "."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=down.gif&version=4.7.5")."' alt='â†“' title='".'Nach unten'."'> ":""),($Df==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.7.5")."' alt='x' title='".'Entfernen'."'>":"");}}function
...
as$y=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($y)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($y))."'>".'Ã„ndern'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Trigger hinzufÃ¼gen'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('Datenbankschema',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Rh=array();$Sh=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$De,PREG_SET_ORDER);foreach($De
...
';$Wc=true;foreach($jg
as$y=>$X){if($y!=""&&$X>1){echo($Wc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$y%")."'>".h($y)."</a>";$Wc=false;}}}elseif(isset($_GET["privileges"])){page_header('Rechte');echo'<p class="links"><a href="'.h(ME).'user=">'.'Benutzer erstellen'."</a>";$G=$g->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$ld=$G;if(!$G)$G=$g->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($ld?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".'Benutzer'."<th>".'Server'."<th></thead>\n";while($I=$G->fetch_assoc())echo'<tr'.odd().'><td>'.h($I["User"])."<td>".h($I["Host"]).'<td><a href="'.h(ME.'user='.urlencode($I["User"]).'&host='.urlencode($I["Host"])).'">'.'Bearbeiten'."</a>\n";if(!$ld||DB!="")echo"<tr".odd()."><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".'Bearbeiten'."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$zd=&get_session("queries");$yd=&$zd[DB];if(!$n&&$_POST["clear"]){$yd=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?'Importieren':'SQL-Kommando'),$n);if(!$n&&$_POST){$id=false;if(!isset($_GET["import"]))$F=$_POST["query"];elseif($_POST["webfile"]){$_h=$b->importServerPath();$id=@fopen((file_exists($_h)?$_h:"compress.zlib://$_h.gz"),"rb");$F=($id?fread($id,1e6):false);}else$F=get_file("sql_file",true);if(is_string($F)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($F)+memory_get_usage()+8e6));if($F!=""&&strlen($F)<1e6){$ug=$F.(preg_match("~;[ \t\r\n]*\$~",$F)?"":";");if(!$yd||reset(end($yd))!=$ug){restart_session();$yd[]=array($ug,time());set_session("queries",$zd);stop_session();}}$xh="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$Wb=";";$C=0;$tc=true;$h=connect();if(is_object($h)&&DB!=""){$h->select_db(DB);if($_GET["ns"]!="")set_schema($_GET["ns"],$h);}$tb=0;$yc=array();$Qf='[\'"'.($x=="sql"?'`#':($x=="sqlite"?'`[':($x=="mssql"?'[':''))).']|/\*|-- |$'.($x=="pgsql"?'|\$[^$]*\$':'');$qi=microtime(true);parse_str($_COOKIE["adminer_export"],$xa);$kc=$b->dumpFormat();unset($kc["sql"]);while($F!=""){if(!$C&&preg_match("~^$xh*+DELIMITER\\s+(\\S+)~i",$F,$A)){$Wb=$A[1];$F=substr($F,strlen($A[0]));}else{preg_match('('.preg_quote($Wb)."\\s*|$Qf)",$F,$A,PREG_OFFSET_CAPTURE,$C);list($gd,$eg)=$A[0];if(!$gd&&$id&&!feof($id))$F.=fread($id,1e5);else{if(!$gd&&rtrim($F)=="")break;$C=$eg+strlen($gd);if($gd&&rtrim($gd)!=$Wb){while(preg_match('('.($gd=='/*'?'\*/':($gd=='['?']':(preg_match('~^-- |^#~',$gd)?"\n":preg_quote($gd)."|\\\\."))).'|$)s',$F,$A,PREG_OFFSET_CAPTURE,$C)){$Yg=$A[0][0];if(!$Yg&&$id&&!feof($id))$F.=fread($id,1e5);else{$C=$A[0][1]+strlen($Yg);if($Yg[0]!="\\")break;}}}else{$tc=false;$ug=substr($F,0,$eg);$tb++;$mg="<pre id='sql-$tb'><code class='jush-$x'>".$b->sqlCommandQuery($ug)."</code></pre>\n";if($x=="sqlite"&&preg_match("~^$xh*+ATTACH\\b~i",$ug,$A)){echo$mg,"<p class='error'>".'ATTACH queries are not supported.'."\n";$yc[]=" <a href='#sql-$tb'>$tb</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$mg;ob_flush();flush();}$Dh=microtime(true);if($g->multi_query($ug)&&is_object($h)&&preg_match("~^$xh*+USE\\b~i",$ug))$h->query($ug);do{$G=$g->store_result();if($g->error){echo($_POST["only_errors"]?$mg:""),"<p class='error'>".'Fehler in der SQL-Abfrage'.($g->errno?" ($g->errno)":"").": ".error()."\n";$yc[]=" <a href='#sql-$tb'>$tb</a>";if($_POST["error_stops"])break
2;}else{$ei=" <span class='time'>(".format_time($Dh).")</span>".(strlen($ug)<1000?" <a href='".h(ME)."sql=".urlencode(trim($ug))."'>".'Bearbeiten'."</a>":"");$za=$g->affected_rows;$dj=($_POST["only_errors"]?"":$m->warnings());$ej="warnings-$tb";if($dj)$ei.=", <a href='#$ej'>".'Warnings'."</a>".script("qsl('a').onclick = partial(toggle, '$ej');","");$Fc=null;$Gc="explain-$tb";if(is_object($G)){$z=$_POST["limit"];$Cf=select($G,$h,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$gf=$G->num_rows;echo"<p>".($gf?($z&&$gf>$z?sprintf('%d / ',$z):"").lang(array('%d Datensatz','%d DatensÃ¤tze'),$gf):""),$ei;if($h&&preg_match("~^($xh|\\()*+SELECT\\b~i",$ug)&&($Fc=explain($h,$ug)))echo", <a href='#$Gc'>Explain</a>".script("qsl('a').onclick = partial(toggle, '$Gc');","");$t="export-$tb";echo", <a href='#$t'>".'Exportieren'."</a>".script("qsl('a').onclick = partial(toggle, '$t');","")."<span id='$t' class='hidden'>: ".html_select("output",$b->dumpOutput(),$xa["output"])." ".html_select("format",$kc,$xa["format"])."<input type='hidden' name='query' value='".h($ug)."'>"." <input type='submit' name='export' value='".'Exportieren'."'><input type='hidden' name='token' value='$oi'></span>\n"."</form>\n";}}else{if(preg_match("~^$xh*+(CREATE|DROP|ALTER)$xh++(DATABASE|SCHEMA)\\b~i",$ug)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($g->info)."'>".lang(array('Abfrage ausgefÃ¼hrt, %d Datensatz betroffen.','Abfrage ausgefÃ¼hrt, %d DatensÃ¤tze betroffen.'),$za)."$ei\n";}echo($dj?"<div id='$ej' class='hidden'>\n$dj</div>\n":"");if($Fc){echo"<div id='$Gc' class='hidden'>\n";select($Fc,$h,$Cf);echo"</div>\n";}}$Dh=microtime(true);}while($g->next_result());}$F=substr($F,$C);$C=0;}}}}if($tc)echo"<p class='message'>".'Kein Kommando vorhanden.'."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".lang(array('SQL-Abfrage erfolgreich ausgefÃ¼hrt.','%d SQL-Abfragen erfolgreich ausgefÃ¼hrt.'),$tb-count($yc))," <span class='time'>(".format_time($qi).")</span>\n";}elseif($yc&&$tb>1)echo"<p class='error'>".'Fehler in der SQL-Abfrage'.": ".implode("",$yc)."\n";}else
echo"<p class='error'>".upload_error($F)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$Cc="<input type='submit' value='".'AusfÃ¼hren'."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$ug=$_GET["sql"];if($_POST)$ug=$_POST["query"];elseif($_GET["history"]=="all")$ug=$yd;elseif($_GET["history"]!="")$ug=$yd[$_GET["history"]][0];echo"<p>";textarea("query",$ug,20);echo
script(($_POST?"":"qs('textarea').focus();\n")."qs('#form').onsubmit = partial(sqlSubmit, qs('#form'), '".remove_from_uri("sql|limit|error_stops|only_errors")."');"),"<p>$Cc\n",'DatensÃ¤tze begrenzen'.": <input type='number' name='limit' class='size' value='".h($_POST?$_POST["limit"]:$_GET["limit"])."'>\n";}else{echo"<fieldset><legend>".'Datei importieren'."</legend><div>";$rd=(extension_loaded("zlib")?"[.gz]":"");echo(ini_bool("file_uploads")?"SQL$rd (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$Cc":'Importieren von Dateien abgeschaltet.'),"</div></fieldset>\n";$Gd=$b->importServerPath();if($Gd){echo"<fieldset><legend>".'Vom Server'."</legend><div>",sprintf('Webserver Datei %s',"<code>".h($Gd)."$rd</code>"),' <input type="submit" name="webfile" value="'.'Datei ausfÃ¼hren'.'">',"</div></fieldset>\n";}echo"<p>";}echo
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])),'Bei Fehler anhalten')."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])),'Nur Fehler anzeigen')."\n","<input type='hidden' name='token' value='$oi'>\n";if(!isset($_GET["import"])&&$yd){print_fieldset("history",'History',$_GET["history"]!="");for($X=end($yd);$X;$X=prev($yd)){$y=key($yd);list($ug,$ei,$oc)=$X;echo'<a href="'.h(ME."sql=&history=$y").'">'.'Bearbeiten'."</a>"." <span class='time' title='".@date('Y-m-d',$ei)."'>".@date("H:i:s",$ei)."</span>"." <code class='jush-$x'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$ug)))),80,"</code>").($oc?" <span class='time'>($oc)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".'LÃ¶schen'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Alle bearbeiten'."</a>\n","</div></fieldset>\n";}echo'</form>
...
as$B=>$o){if(!isset($o["privileges"][$Ji?"update":"insert"])||$b->fieldName($o)==""||$o["generated"])unset($p[$B]);}if($_POST&&!$n&&!isset($_GET["select"])){$xe=$_POST["referer"];if($_POST["insert"])$xe=($Ji?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$xe))$xe=ME."select=".urlencode($a);$w=indexes($a);$Ei=unique_array($_GET["where"],$w);$xg="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($xe,'Datensatz wurde gelÃ¶scht.',$m->delete($a,$xg,!$Ei));else{$N=array();foreach($p
as$B=>$o){$X=process_input($o);if($X!==false&&$X!==null)$N[idf_escape($B)]=$X;}if($Ji){if(!$N)redirect($xe);queries_redirect($xe,'Datensatz wurde geÃ¤ndert.',$m->update($a,$N,$xg,!$Ei));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$G=$m->insert($a,$N);$oe=($G?last_id():0);queries_redirect($xe,sprintf('Datensatz%s wurde eingefÃ¼gt.',($oe?" $oe":"")),$G);}}}$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($p
...
';if($a==""&&!$_POST)echo
script("focus(qs('#form')['name']);");echo($vc?"<select name='Engine'>".optionlist(array(""=>"(".'Speicher-Engine'.")")+$vc,$I["Engine"])."</select>".on_help("getTarget(event).value",1).script("qsl('select').onchange = helpClose;"):""),' ',($pb&&!preg_match("~sqlite|mssql~",$x)?html_select("Collation",array(""=>"(".'Kollation'.")")+$pb,$I["Collation"]):""),' <input type="submit" value="Speichern">
';}echo'
...
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="Entfernen">',confirm(sprintf('Drop %s?',$a));}if(support("partitioning")){$Tf=preg_match('~RANGE|LIST~',$I["partition_by"]);print_fieldset("partition",'Partitionieren um',$I["partition_by"]);echo'<p>
...
Partitionen: <input type="number" name="partitions" class="size',($Tf||!$I["partition_by"]?" hidden":""),'" value="',h($I["partitions"]),'">
...
<th><noscript>',"<input type='image' class='icon' name='add[0]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.7.5")."' alt='+' title='".'HinzufÃ¼gen'."'>",'</noscript>
</thead>
';if($kg){echo"<tr><td>PRIMARY<td>";foreach($kg["columns"]as$y=>$e){echo
select_input(" disabled",$p,$e),"<label><input disabled type='checkbox'>".'absteigend'."</label> ";}echo"<td><td>\n";}$ce=1;foreach($I["indexes"]as$v){if(!$_POST["drop_col"]||$ce!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$ce][type]",array(-1=>"")+$Jd,$v["type"],($ce==count($I["indexes"])?"indexesAddRow.call(this);":1),"label-type"),"<td>";ksort($v["columns"]);$s=1;foreach($v["columns"]as$y=>$e){echo"<span>".select_input(" name='indexes[$ce][columns][$s]' title='".'Spalte'."'",($p?array_combine($p,$p):$p),$e,"partial(".($s==count($v["columns"])?"indexesAddColumn":"indexesChangeColumn").", '".js_escape($x=="sql"?"":$_GET["indexes"]."_")."')"),($x=="sql"||$x=="mssql"?"<input type='number' name='indexes[$ce][lengths][$s]' class='size' value='".h($v["lengths"][$y])."' title='".'LÃ¤nge'."'>":""),(support("descidx")?checkbox("indexes[$ce][descs][$s]",1,$v["descs"][$y],'absteigend'):"")," </span>";$s++;}echo"<td><input name='indexes[$ce][name]' value='".h($v["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$ce]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.7.5")."' alt='x' title='".'Entfernen'."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");}$ce++;}echo'</table>
</div>
<p>
<input type="submit" value="Speichern">
<input type="hidden" name="token" value="',$oi,'">
...
as$l){if(count($k)==1||$l!=""){if(!create_database($l,$I["collation"]))$Jh=false;$ne=$l;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($ne),'Datenbank wurde erstellt.',$Jh);}}else{if(!$I["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($B).(preg_match('~^[a-z0-9_]+$~i',$I["collation"])?" COLLATE $I[collation]":""),substr(ME,0,-1),'Datenbank wurde geÃ¤ndert.');}}page_header(DB!=""?'Datenbank Ã¤ndern':'Datenbank erstellen',$n,array(),h(DB));$pb=collations();$B=DB;if($_POST)$B=$I["name"];elseif(DB!="")$I["collation"]=db_collation(DB,$pb);elseif($x=="sql"){foreach(get_vals("SHOW GRANTS")as$ld){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\.\*)?~',$ld,$A)&&$A[1]){$B=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
';if(DB!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'>".confirm(sprintf('Drop %s?',DB))."\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.7.5")."' alt='+' title='".'HinzufÃ¼gen'."'>\n";echo'<input type="hidden" name="token" value="',$oi,'">
</form>
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$B=$_GET["name"];$I=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$Le=($_POST["drop"]?'FremdschlÃ¼ssel wurde entfernt.':($B!=""?'FremdschlÃ¼ssel wurde geÃ¤ndert.':'FremdschlÃ¼ssel wurde erstellt.'));$xe=ME."table=".urlencode($a);if(!$_POST["drop"]){$I["source"]=array_filter($I["source"],'strlen');ksort($I["source"]);$Xh=array();foreach($I["source"]as$y=>$X)$Xh[$y]=$I["target"][$y];$I["target"]=$Xh;}if($x=="sqlite")queries_redirect($xe,$Le,recreate_table($a,$a,array(),array(),array(" $B"=>($_POST["drop"]?"":" ".format_foreign_key($I)))));else{$c="ALTER TABLE ".table($a);$fc="\nDROP ".($x=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($B);if($_POST["drop"])query_redirect($c.$fc,$xe,$Le);else{query_redirect($c.($B!=""?"$fc,":"")."\nADD".format_foreign_key($I),$xe,$Le);$n='Quell- und Zielspalten mÃ¼ssen vom gleichen Datentyp sein, es muss unter den Zielspalten ein Index existieren und die referenzierten Daten mÃ¼ssen existieren.'."<br>$n";}}}page_header('FremdschlÃ¼ssel',$n,array("table"=>$a),h($a));if($_POST){ksort($I["source"]);if($_POST["add"])$I["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$I["target"]=array();}elseif($B!=""){$ed=foreign_keys($a);$I=$ed[$B];$I["source"][]="";}else{$I["table"]=$a;$I["source"]=array("");}echo'
<form action="" method="post">
';$wh=array_keys(fields($a));if($I["db"]!="")$g->select_db($I["db"]);if($I["ns"]!="")set_schema($I["ns"]);$Dg=array_keys(array_filter(table_status('',true),'fk_support'));$Xh=($a===$I["table"]?$wh:array_keys(fields(in_array($I["table"],$Dg)?$I["table"]:reset($Dg))));$rf="this.form['change-js'].value = '1'; this.form.submit();";echo"<p>".'Zieltabelle'.": ".html_select("table",$Dg,$I["table"],$rf)."\n";if($x=="pgsql")echo'Schema'.": ".html_select("ns",$b->schemas(),$I["ns"]!=""?$I["ns"]:$_GET["ns"],$rf);elseif($x!="sqlite"){$Rb=array();foreach($b->databases()as$l){if(!information_schema($l))$Rb[]=$l;}echo'DB'.": ".html_select("db",$Rb,$I["db"]!=""?$I["db"]:$_GET["db"],$rf);}echo'<input type="hidden" name="change-js" value="">
...
';if($B!=""){echo'<input type="submit" name="drop" value="Entfernen">',confirm(sprintf('Drop %s?',$B));}echo'<input type="hidden" name="token" value="',$oi,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$I=$_POST;$Gf="VIEW";if($x=="pgsql"&&$a!=""){$O=table_status($a);$Gf=strtoupper($O["Engine"]);}if($_POST&&!$n){$B=trim($I["name"]);$Ga=" AS\n$I[select]";$xe=ME."table=".urlencode($B);$Le='View wurde geÃ¤ndert.';$T=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$B&&$x!="sqlite"&&$T=="VIEW"&&$Gf=="VIEW")query_redirect(($x=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($B).$Ga,$xe,$Le);else{$Zh=$B."_adminer_".uniqid();drop_create("DROP $Gf ".table($a),"CREATE $T ".table($B).$Ga,"DROP $T ".table($B),"CREATE $T ".table($Zh).$Ga,"DROP $T ".table($Zh),($_POST["drop"]?substr(ME,0,-1):$xe),'View wurde entfernt.',$Le,'View wurde erstellt.',$a,$B);}}if(!$_POST&&$a!=""){$I=view($a);$I["name"]=$a;$I["materialized"]=($Gf!="VIEW");if(!$n)$n=error();}page_header(($a!=""?'View Ã¤ndern':'View erstellen'),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>Jede<td><input type="number" name="INTERVAL_VALUE" value="',h($I["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Ud,$I["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Fh,$I["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($I["EVENT_COMMENT"]),'" data-maxlength="64">
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$B=$_GET["name"];$zi=trigger_options();$I=(array)trigger($B)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$zi["Timing"])&&in_array($_POST["Event"],$zi["Event"])&&in_array($_POST["Type"],$zi["Type"])){$pf=" ON ".table($a);$fc="DROP TRIGGER ".idf_escape($B).($x=="pgsql"?$pf:"");$xe=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($fc,$xe,'Trigger wurde entfernt.');else{if($B!="")queries($fc);queries_redirect($xe,($B!=""?'Trigger wurde geÃ¤ndert.':'Trigger wurde erstellt.'),queries(create_trigger($pf,$_POST)));if($B!="")queries(create_trigger($pf,$I+array("Type"=>reset($zi["Type"]))));}}$I=$_POST;}page_header(($B!=""?'Trigger Ã¤ndern'.": ".h($B):'Trigger erstellen'),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0" class="layout">
<tr><th>Zeitpunkt<td>',html_select("Timing",$zi["Timing"],$I["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>Ereignis<td>',html_select("Event",$zi["Event"],$I["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$zi["Event"])?" <input name='Of' value='".h($I["Of"])."' class='hidden'>":""),'<tr><th>Typ<td>',html_select("Type",$zi["Type"],$I["Type"]),'</table>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".'Rechte'.doc_link(array('sql'=>"grant.html#priv_level"));$s=0;foreach($md
as$if=>$ld){echo'<th>'.($if!="*.*"?"<input name='objects[$s]' value='".h($if)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Routine',)as$Bb=>$Xb){foreach((array)$pg[$Bb]as$og=>$ub){echo"<tr".odd()."><td".($Xb?">$Xb<td":" colspan='2'").' lang="en" title="'.h($ub).'">'.h($og);$s=0;foreach($md
...
<input type="submit" value="Speichern">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Entfernen">',confirm(sprintf('Drop %s?',"$ha@$_GET[host]"));}echo'<input type="hidden" name="token" value="',$oi,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$je=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$je++;}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$je),$je||!$_POST["kill"]);}page_header('Prozessliste',$n);echo'
<form action="" method="post">
<div class="scrollable">
...
as$B=>$X){$X=process_input($p[$B]);if($X!==null&&($_POST["clone"]||$X!==false))$N[idf_escape($B)]=($X!==false?$X:idf_escape($B));}}if($_POST["delete"]||$N){if($_POST["clone"])$F="INTO ".table($a)." (".implode(", ",array_keys($N)).")\nSELECT ".implode(", ",$N)."\nFROM ".table($a);if($_POST["all"]||($kg&&is_array($_POST["check"]))||$Yd){$G=($_POST["delete"]?$m->delete($a,$jj):($_POST["clone"]?queries("INSERT $F$jj"):$m->update($a,$N,$jj)));$za=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$fj="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$G=($_POST["delete"]?$m->delete($a,$fj,1):($_POST["clone"]?queries("INSERT".limit1($a,$F,$fj)):$m->update($a,$N,$fj,1)));if(!$G)break;$za+=$g->affected_rows;}}}$Le=sprintf('%d Artikel betroffen.',$za);if($_POST["clone"]&&$G&&$za==1){$oe=last_id();if($oe)$Le=sprintf('Datensatz%s wurde eingefÃ¼gt.'," $oe");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$Le,$G);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n='Ctrl+Klick zum Bearbeiten des Wertes.';else{$G=true;$za=0;foreach($_POST["val"]as$Fi=>$I){$N=array();foreach($I
as$y=>$X){$y=bracket_escape($y,1);$N[idf_escape($y)]=(preg_match('~char|text~',$p[$y]["type"])||$X!=""?$b->processInput($p[$y],$X):"NULL");}$G=$m->update($a,$N," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Fi,$p),!$Yd&&!$kg," ");if(!$G)break;$za+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d Artikel betroffen.',$za),$G);}}elseif(!is_string($Tc=get_file("csv_file",true)))$n=upload_error($Tc);elseif(!preg_match('~~u',$Tc))$n='Die Datei muss UTF-8 kodiert sein.';else{cookie("adminer_import","output=".urlencode($ya["output"])."&format=".urlencode($_POST["separator"]));$G=true;$rb=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\r\n]+)+~',$Tc,$De);$za=count($De[0]);$m->begin();$L=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$J=array();foreach($De[0]as$y=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$L]*)$L~",$X.$L,$Ee);if(!$y&&!array_diff($Ee[1],$rb)){$rb=$Ee[1];$za--;}else{$N=array();foreach($Ee[1]as$s=>$nb)$N[idf_escape($rb[$s])]=($nb==""&&$p[$rb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$nb))));$J[]=$N;}}$G=(!$J||$m->insertUpdate($a,$J,$kg));if($G)$G=$m->commit();queries_redirect(remove_from_uri("page"),lang(array('%d Datensatz wurde importiert.','%d DatensÃ¤tze wurden importiert.'),$za),$G);$m->rollback();}}}$Ph=$b->tableName($R);if(is_ajax()){page_headers();ob_start();}else
page_header('Daten zeigen von'.": $Ph",$n);$N=null;if(isset($Sg["insert"])||!support("table")){$N="";foreach((array)$_GET["where"]as$X){if($ed[$X["col"]]&&count($ed[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$N.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($R,$N);if(!$f&&support("table"))echo"<p class='error'>".'Auswahl der Tabelle fehlgeschlagen'.($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($K,$f);$b->selectSearchPrint($Z,$f,$w);$b->selectOrderPrint($_f,$f,$w);$b->selectLimitPrint($z);$b->selectLengthPrint($di);$b->selectActionPrint($w);echo"</form>\n";$D=$_GET["page"];if($D=="last"){$hd=$g->result(count_rows($a,$Z,$Yd,$nd));$D=floor(max(0,$hd-1)/$z);}$eh=$K;$od=$nd;if(!$eh){$eh[]="*";$Cb=convert_fields($f,$p,$K);if($Cb)$eh[]=substr($Cb,2);}foreach($K
as$y=>$X){$o=$p[idf_unescape($X)];if($o&&($Ga=convert_field($o)))$eh[$y]="$Ga AS $X";}if(!$Yd&&$Hi){foreach($Hi
as$y=>$X){$eh[]=idf_escape($y);if($od)$od[]=idf_escape($y);}}$G=$m->select($a,$eh,$Z,$od,$_f,$z,$D,true);if(!$G)echo"<p class='error'>".error()."\n";else{if($x=="mssql"&&$D)$G->seek($z*$D);$sc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$J=array();while($I=$G->fetch_assoc()){if($D&&$x=="oracle")unset($I["RNUM"]);$J[]=$I;}if($_GET["page"]!="last"&&$z!=""&&$nd&&$Yd&&$x=="sql")$hd=$g->result(" SELECT FOUND_ROWS()");if(!$J)echo"<p class='message'>".'Keine DatensÃ¤tze.'."\n";else{$Qa=$b->backwardKeys($a,$Ph);echo"<div class='scrollable'>","<table id='table' cellspacing='0' class='nowrap checkable'>",script("mixin(qs('#table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true), onkeydown: editingKeydown});"),"<thead><tr>".(!$nd&&$K?"":"<td><input type='checkbox' id='all-page' class='jsonly'>".script("qs('#all-page').onclick = partial(formCheck, /check/);","")." <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'Ã„ndern'."</a>");$Xe=array();$kd=array();reset($K);$zg=1;foreach($J[0]as$y=>$X){if(!isset($Hi[$y])){$X=$_GET["columns"][key($K)];$o=$p[$K?($X?$X["col"]:current($K)):$y];$B=($o?$b->fieldName($o,$zg):($X["fun"]?"*":$y));if($B!=""){$zg++;$Xe[$y]=$B;$e=idf_escape($y);$Bd=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);$Xb="&desc%5B0%5D=1";echo"<th>".script("mixin(qsl('th'), {onmouseover: partial(columnMouse), onmouseout: partial(columnMouse, ' hidden')});",""),'<a href="'.h($Bd.($_f[0]==$e||$_f[0]==$y||(!$_f&&$Yd&&$nd[0]==$e)?$Xb:'')).'">';echo
apply_sql_function($X["fun"],$B)."</a>";echo"<span class='column hidden'>","<a href='".h($Bd.$Xb)."' title='".'absteigend'."' class='text'> â†“</a>";if(!$X["fun"]){echo'<a href="#fieldset-search" title="'.'Suchen'.'" class="text jsonly"> =</a>',script("qsl('a').onclick = partial(selectSearch, '".js_escape($y)."');");}echo"</span>";}$kd[$y]=$X["fun"];next($K);}}$ue=array();if($_GET["modify"]){foreach($J
as$I){foreach($I
as$y=>$X)$ue[$y]=max($ue[$y],min(40,strlen(utf8_decode($X))));}}echo($Qa?"<th>".'Relationen':"")."</thead>\n";if(is_ajax()){if($z%2==1&&$D%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($J,$ed)as$We=>$I){$Ei=unique_array($J[$We],$w);if(!$Ei){$Ei=array();foreach($J[$We]as$y=>$X){if(!preg_match('~^(COUNT\((\*|(DISTINCT )?`(?:[^`]|``)+`)\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\(`(?:[^`]|``)+`\))$~',$y))$Ei[$y]=$X;}}$Fi="";foreach($Ei
as$y=>$X){if(($x=="sql"||$x=="pgsql")&&preg_match('~char|text|enum|set~',$p[$y]["type"])&&strlen($X)>64){$y=(strpos($y,'(')?$y:idf_escape($y));$y="MD5(".($x!='sql'||preg_match("~^utf8~",$p[$y]["collation"])?$y:"CONVERT($y USING ".charset($g).")").")";$X=md5($X);}$Fi.="&".($X!==null?urlencode("where[".bracket_escape($y)."]")."=".urlencode($X):"null%5B%5D=".urlencode($y));}echo"<tr".odd().">".(!$nd&&$K?"":"<td>".checkbox("check[]",substr($Fi,1),in_array(substr($Fi,1),(array)$_POST["check"])).($Yd||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Fi)."' class='edit'>".'bearbeiten'."</a>"));foreach($I
...
as$de=>$W)$_.=where_link($s++,$de,$W);}$X=select_value($X,$_,$o,$di);$t=h("val[$Fi][".bracket_escape($y)."]");$Y=$_POST["val"][$Fi][bracket_escape($y)];$nc=!is_array($I[$y])&&is_utf8($X)&&$J[$We][$y]==$I[$y]&&!$kd[$y];$ci=preg_match('~text|lob~',$o["type"]);echo"<td id='$t'";if(($_GET["modify"]&&$nc)||$Y!==null){$sd=h($Y!==null?$Y:$I[$y]);echo">".($ci?"<textarea name='$t' cols='30' rows='".(substr_count($I[$y],"\n")+1)."'>$sd</textarea>":"<input name='$t' value='$sd' size='$ue[$y]'>");}else{$ze=strpos($X,"<i>â€¦</i>");echo" data-text='".($ze?2:($ci?1:0))."'".($nc?"":" data-warning='".h('Benutzen Sie den Link zum Bearbeiten dieses Wertes.')."'").">$X</td>";}}}if($Qa)echo"<td>";$b->backwardKeysPrint($Qa,$J[$We]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n","</div>\n";}if(!is_ajax()){if($J||$D){$Bc=true;if($_GET["page"]!="last"){if($z==""||(count($J)<$z&&($J||!$D)))$hd=($D?$D*$z:0)+count($J);elseif($x!="sql"||!$Yd){$hd=($Yd?false:found_rows($R,$Z));if($hd<max(1e4,2*($D+1)*$z))$hd=reset(slow_query(count_rows($a,$Z,$Yd,$nd)));else$Bc=false;}}$Mf=($z!=""&&($hd===false||$hd>$z||$D));if($Mf){echo(($hd===false?count($J)+1:$hd-$D*$z)>$z?'<p><a href="'.h(remove_from_uri("page")."&page=".($D+1)).'" class="loadmore">'.'Mehr Daten laden'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$z).", '".'Lade'."â€¦');",""):''),"\n";}}echo"<div class='footer'><div>\n";if($J||$D){if($Mf){$Ge=($hd===false?$D+(count($J)>=$z?2:1):floor(($hd-1)/$z));echo"<fieldset>";if($x!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".'Seite'."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'Seite'."', '".($D+1)."')); return false; };"),pagination(0,$D).($D>5?" â€¦":"");for($s=max(1,$D-4);$s<min($Ge,$D+5);$s++)echo
pagination($s,$D);if($Ge>0){echo($D+5<$Ge?" â€¦":""),($Bc&&$hd!==false?pagination($Ge,$D):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Ge'>".'letzte'."</a>");}}else{echo"<legend>".'Seite'."</legend>",pagination(0,$D).($D>1?" â€¦":""),($D?pagination($D,$D):""),($Ge>$D?pagination($D+1,$D).($Ge>$D+1?" â€¦":""):"");}echo"</fieldset>\n";}echo"<fieldset>","<legend>".'Gesamtergebnis'."</legend>";$cc=($Bc?"":"~ ").$hd;echo
checkbox("all",1,0,($hd!==false?($Bc?"":"~ ").lang(array('%d Datensatz','%d DatensÃ¤tze'),$hd):""),"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$cc' : checked); selectCount('selected2', this.checked || !checked ? '$cc' : checked);")."\n","</fieldset>\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Ã„ndern</legend><div>
...
';}$fd=$b->dumpFormat();foreach((array)$_GET["columns"]as$e){if($e["fun"]){unset($fd['sql']);break;}}if($fd){print_fieldset("export",'Exportieren'." <span id='selected2'></span>");$Kf=$b->dumpOutput();echo($Kf?html_select("output",$Kf,$ya["output"])." ":""),html_select("format",$fd,$ya["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($sc,'strlen'),$f);}echo"</div></div>\n";if($b->selectImportPrint()){echo"<div>","<a href='#import'>".'Importieren'."</a>",script("qsl('a').onclick = partial(toggle, 'import');",""),"<span id='import' class='hidden'>: ","<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$ya["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","</span>","</div>";}echo"<input type='hidden' name='token' value='$oi'>\n","</form>\n",(!$nd&&$K?"":script("tableCheck();"));}}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$O=isset($_GET["status"]);page_header($O?'Status':'Variablen');$Wi=($O?show_status():show_variables());if(!$Wi)echo"<p class='message'>".'Keine DatensÃ¤tze.'."\n";else{echo"<table cellspacing='0'>\n";foreach($Wi
...
as$y=>$X)json_row("sum-$y",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".number($_POST["kill"]));else{foreach(count_tables($b->databases())as$l=>$X){json_row("tables-$l",$X);json_row("size-$l",db_size($l));}json_row("");}exit;}else{$Vh=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Vh&&!$n&&!$_POST["search"]){$G=true;$Le="";if($x=="sql"&&$_POST["tables"]&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$G=truncate_tables($_POST["tables"]);$Le='Tabellen wurden geleert (truncate).';}elseif($_POST["move"]){$G=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Le='Tabellen verschoben.';}elseif($_POST["copy"]){$G=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Le='Tabellen wurden kopiert.';}elseif($_POST["drop"]){if($_POST["views"])$G=drop_views($_POST["views"]);if($G&&$_POST["tables"])$G=drop_tables($_POST["tables"]);$Le='Tabellen wurden entfernt (drop).';}elseif($x!="sql"){$G=($x=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$Le='Tabellen wurden optimiert.';}elseif(!$_POST["tables"])$Le='Keine Tabellen.';elseif($G=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($I=$G->fetch_assoc())$Le.="<b>".h($I["Table"])."</b>: ".h($I["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$Le,$G);}page_header(($_GET["ns"]==""?'Datenbank'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$n,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'Tabellen und Views'."</h3>\n";$Uh=tables_list();if(!$Uh)echo"<p class='message'>".'Keine Tabellen.'."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".'Suche in Tabellen'." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'>",script("qsl('input').onkeydown = partialArg(bodyKeydown, 'search');","")," <input type='submit' name='search' value='".'Suchen'."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!=""){$_GET["where"][0]["op"]="LIKE %%";search_tables();}}echo"<div class='scrollable'>\n","<table cellspacing='0' class='nowrap checkable'>\n",script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"),'<thead><tr class="wrap">','<td><input id="check-all" type="checkbox" class="jsonly">'.script("qs('#check-all').onclick = partial(formCheck, /^(tables|views)\[/);",""),'<th>'.'Tabelle','<td>'.'Speicher-Engine'.doc_link(array('sql'=>'storage-engines.html')),'<td>'.'Kollation'.doc_link(array('sql'=>'charset-charsets.html','mariadb'=>'supported-character-sets-and-collations/')),'<td>'.'DatengrÃ¶ÃŸe'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'functions-admin.html#FUNCTIONS-ADMIN-DBOBJECT','oracle'=>'REFRN20286')),'<td>'.'IndexgrÃ¶ÃŸe'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'functions-admin.html#FUNCTIONS-ADMIN-DBOBJECT')),'<td>'.'Freier Bereich'.doc_link(array('sql'=>'show-table-status.html')),'<td>'.'Auto-Inkrement'.doc_link(array('sql'=>'example-auto-increment.html','mariadb'=>'auto_increment/')),'<td>'.'DatensÃ¤tze'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'catalog-pg-class.html#CATALOG-PG-CLASS','oracle'=>'REFRN20286')),(support("comment")?'<td>'.'Kommentar'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'functions-info.html#FUNCTIONS-INFO-COMMENT-TABLE')):''),"</thead>\n";$S=0;foreach($Uh
as$B=>$T){$Zi=($T!==null&&!preg_match('~table~i',$T));$t=h("Table-".$B);echo'<tr'.odd().'><td>'.checkbox(($Zi?"views[]":"tables[]"),$B,in_array($B,$Vh,true),"","","",$t),'<th>'.(support("table")||support("indexes")?"<a href='".h(ME)."table=".urlencode($B)."' title='".'Struktur anzeigen'."' id='$t'>".h($B).'</a>':h($B));if($Zi){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($B).'" title="'.'View Ã¤ndern'.'">'.(preg_match('~materialized~i',$T)?'Materialized view':'View').'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($B).'" title="'.'Daten auswÃ¤hlen'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Tabelle Ã¤ndern'),"Index_length"=>array("indexes",'Indizes Ã¤ndern'),"Data_free"=>array("edit",'Neuer Datensatz'),"Auto_increment"=>array("auto_increment=1&create",'Tabelle Ã¤ndern'),"Rows"=>array("select",'Daten auswÃ¤hlen'),)as$y=>$_){$t=" id='$y-".h($B)."'";echo($_?"<td align='right'>".(support("table")||$y=="Rows"||(support("indexes")&&$y!="Data_length")?"<a href='".h(ME."$_[0]=").urlencode($B)."'$t title='$_[1]'>?</a>":"<span$t>?</span>"):"<td id='$y-".h($B)."'>");}$S++;}echo(support("comment")?"<td id='Comment-".h($B)."'>":"");}echo"<tr><td><th>".sprintf('%d insgesamt',count($Uh)),"<td>".h($x=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".h(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$y)echo"<td align='right' id='sum-$y'>";echo"</table>\n","</div>\n";if(!information_schema(DB)){echo"<div class='footer'><div>\n";$Ti="<input type='submit' value='".'Vacuum'."'> ".on_help("'VACUUM'");$wf="<input type='submit' name='optimize' value='".'Optimieren'."'> ".on_help($x=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'");echo"<fieldset><legend>".'AusgewÃ¤hlte'." <span id='selected'></span></legend><div>".($x=="sqlite"?$Ti:($x=="pgsql"?$Ti.$wf:($x=="sql"?"<input type='submit' value='".'Analysieren'."'> ".on_help("'ANALYZE TABLE'").$wf."<input type='submit' name='check' value='".'PrÃ¼fen'."'> ".on_help("'CHECK TABLE'")."<input type='submit' name='repair' value='".'Reparieren'."'> ".on_help("'REPAIR TABLE'"):"")))."<input type='submit' name='truncate' value='".'Leeren (truncate)'."'> ".on_help($x=="sqlite"?"'DELETE'":"'TRUNCATE".($x=="pgsql"?"'":" TABLE'")).confirm()."<input type='submit' name='drop' value='".'Entfernen'."'>".on_help("'DROP TABLE'").confirm()."\n";$k=(support("scheme")?$b->schemas():$b->databases());if(count($k)!=1&&$x!="sqlite"){$l=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'In andere Datenbank verschieben'.": ",($k?html_select("target",$k,$l):'<input name="target" value="'.h($l).'" autocapitalize="off">')," <input type='submit' name='move' value='".'Verschieben'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopieren'."'> ".checkbox("overwrite",1,$_POST["overwrite"],'overwrite'):""),"\n";}echo"<input type='hidden' name='all' value=''>";echo
